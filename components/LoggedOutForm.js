...
function LoggedOutForm(props) {
  ...
  async function TheLoginFunction(event) {
    ...
    const theResponse = await Axios.post(`${API_BASE_URL}/user`, { theUserName: userName, thePassword: password })
    if (theResponse.data.thePasswordMatching) {
      localStorage.setItem("localLoggedIn", theResponse.data.thePasswordMatching)
